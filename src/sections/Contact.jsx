...
const Contact = () => {
  ...
  const sendEmail = async (e) => {
    ...
    try {
      const res = await fetch("https://formsubmit.co/otenedaniel007@gmail.com", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    ...
    >
      ...
      <form onSubmit={sendEmail} className="w-fit md:w-1/2 space-y-4 p-2 md:p-10">
        ...
        <div>
          ...
          <textarea
            name="message"
            placeholder="How you want the site to look..."
            className="w-full mt-1 p-2 rounded-md bg-gray-100 border-gray-200 border"
            rows="4"
            required
          />
      ...
      {isSuccess && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-purple-600 text-white px-4 py-2 rounded-md shadow-lg">
          Message sent successfully!
        </div>
      )}

      {isError && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-red-500 text-white px-4 py-2 rounded-md shadow-lg">
